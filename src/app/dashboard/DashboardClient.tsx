...
import CurrentBlockWidget from '@/components/dashboard/CurrentBlockWidget';
...
interface DashboardClientProps {
  ...
  unresolvedBlocks?: UnresolvedBlockProp[];
}
...
export default function DashboardClient({ 
  ...
  scheduleBlocks = [],
  initialRoutineMode,
  unresolvedBlocks: initialUnresolvedBlocks = []
}: DashboardClientProps) {
  const router = useRouter();
  const [showOverdue, setShowOverdue] = useState(true);
  const [tasks, setTasks] = useState(initialTasks);
  const [showTriageModal, setShowTriageModal] = useState(initialUnresolvedBlocks.length > 0);
  ...
  return (
    <div className="p-8 pb-24 max-w-[1100px]  mx-auto w-full min-h-full flex flex-col">
        ...
        <div className="relative">
          ...
          <div className="relative flex h-[6px] rounded-full overflow-hidden bg-divider">
            {(() => {
              ...
              return scheduleBlocks.map((block, i) => {
                ...
                const prevEnd = i > 0 ? (() => {
                  const [ph, pm] = scheduleBlocks[i - 1].endTime.split(':').map(Number);
                  return ph * 60 + pm;
                })() : timelineStart;
          ...
          <div className="relative flex mt-2.5 min-h-[30px]">
            {(() => {
              ...
              // Find the absolute latest end time string to display at the right edge
              ...
              return (
                <>
                  {scheduleBlocks.map((block, i) => {
                    ...
                    const [eh, em] = block.endTime.split(':').map(Number);
                    const blockStartMin = sh * 60 + sm;
                    const durationMinutes = (eh * 60 + em) - blockStartMin;
                    const widthPct = (durationMinutes / totalMinutes) * 100;
                    const prevEnd = i > 0 ? (() => {
                      const [ph, pm] = scheduleBlocks[i - 1].endTime.split(':').map(Number);
                      return ph * 60 + pm;
                    ...
                    const now = new Date();
                    const currentMinutes = now.getHours() * 60 + now.getMinutes();
                    const isActive = currentMinutes >= blockStartMin && currentMinutes < (eh * 60 + em);

                    return (
                      <div key={block.id} className="contents">
                        {gapPct > 0.5 && <div style={{ width: `${gapPct}%` }} />}
                        <div style={{ width: `${widthPct}%` }} className="min-w-0 pr-2">
      ...
      <CurrentBlockWidget
        ...
        onComplete={async (blockId) => {
          const { markBlockCompleted } = await import('@/app/actions/schedule-tracking.actions');
          await markBlockCompleted(blockId);
        }}
        onEndEarly={async (blockId) => {
          const { markBlockCompletedEarly } = await import('@/app/actions/schedule-tracking.actions');
          await markBlockCompletedEarly(blockId);
        }}
        onInterrupt={async (blockId, remark, category) => {
          const { interruptBlock } = await import('@/app/actions/schedule-tracking.actions');
          await interruptBlock(blockId, remark, category);
        }}
        onExtend={async (currentBlockId, nextBlockId) => {
          const { extendBlock } = await import('@/app/actions/schedule-tracking.actions');
          await extendBlock(currentBlockId, nextBlockId);
        }}
        onPreSkip={async (blockId, remark, category) => {
          const { preSkipBlock } = await import('@/app/actions/schedule-tracking.actions');
          await preSkipBlock(blockId, remark, category);
        }}
        onReplace={(victimBlockId) => {
          // TODO: Phase 2 — open a block selector modal
          console.log('Replace block:', victimBlockId);
        }}
      />
      ...
      {/* Schedule Triage Modal */}
      <TriageModal
        isOpen={showTriageModal}
        unresolvedBlocks={initialUnresolvedBlocks}
        onResolve={async (logId, outcome, remark, remarkCategory) => {
          const { resolveUnresolvedBlock } = await import('@/app/actions/schedule-tracking.actions');
          await resolveUnresolvedBlock(logId, outcome, remark, remarkCategory);
        }}
