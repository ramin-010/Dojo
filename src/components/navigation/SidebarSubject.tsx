    ...
    return (
      <div className="flex justify-center py-2">
        <Link 
          ...
          title={subject.name} 
          className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-hover text-muted hover:text-foreground transition-all duration-200 opacity-20 hover:opacity-100"
        >
    ...
    <div className="mb-0.5">
      <div className="flex items-center justify-between px-1.5 py-1 rounded-lg hover:bg-hover/50 group transition-colors">
        <div className="flex items-center gap-1.5 overflow-hidden flex-1">
          <button 
            className="text-muted/50 hover:text-foreground transition-colors shrink-0 p-0.5 rounded hover:bg-hover/80"
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          >
            <div className="pl-6 pr-1 pb-1">
              ...
              {activeTopics.length === 0 && archivedTopics.length === 0 && (
                <div className="px-2 py-2 text-[11px] text-muted/50 italic">
                  No topics yet
                </div>
              )}
              ...
              {archivedTopics.length > 0 && (
                <div className="mt-1 pt-1 border-t border-border/20">
                  <button 
                    ...
                    className="flex items-center gap-1.5 w-full text-left px-2 py-1.5 text-[11px] font-medium text-muted/60 hover:text-foreground transition-colors rounded-lg hover:bg-hover/50"
                  >
